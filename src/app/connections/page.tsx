  ...
  return (
    <Shell user={user} active="connections">
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
        <form action={syncRelations} className="rounded-[18px] border border-[#263BAA]/12 bg-white p-6">
          <h2 className="text-lg font-medium">Sync from LinkedIn</h2>
          <p className="mt-1 text-sm text-[#2B3355]/55">Pull all 1st-degree connections through the connected account.</p>
          <button disabled={!seat}
            className="mt-4 rounded-lg bg-[#263BAA] px-4 py-2 text-sm font-semibold text-[#1B2559] shadow-[0_0_18px_rgba(38,59,170,.2)] hover:bg-[#1D2E86] disabled:cursor-not-allowed disabled:bg-[#263BAA]/10 disabled:text-[#2B3355]/35 disabled:shadow-none">
            Sync connections
          </button>
          {!seat && (
            <p className="mt-3 text-xs text-[#2B3355]/45">
        ...
        <form action={uploadCsv} className="rounded-[18px] border border-[#263BAA]/12 bg-white p-6">
      ...
      <ul className="mt-3 divide-y divide-[#263BAA]/8 rounded-[18px] border border-[#263BAA]/12 bg-white">
        {batches.length === 0 && (
          <li className="border border-dashed border-[#263BAA]/12 p-8 text-center text-sm text-[#2B3355]/45">
          ...
          return (
            <li key={b.id}>
              <Link href={`/batches/${b.id}`} className="flex items-center gap-4 p-4 text-sm hover:bg-[#263BAA]/5">
                ...
                <span className="rounded border border-[#263BAA]/20 px-1.5 py-0.5 text-[11px] text-[#2B3355]/55">{b.source}</span>
                <span className="tnum w-20 shrink-0 text-[#2B3355]/40">{b.createdAt.toISOString().slice(5, 10)}</span>
                ...
                <span className="tnum shrink-0 text-[#2B3355]/55">
                  {total === 0 ? "importing…"
                    : c.pitchable > 0
