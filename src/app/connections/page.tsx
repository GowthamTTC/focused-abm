    ...
    <Shell user={user} active="connections">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
        <form action={syncRelations} className="rounded-[18px] border border-white/10 bg-[#1F2329] p-5">
          ...
          <p className="mt-1 text-sm text-white/55">Pull all 1st-degree connections through the connected account. One pass, low risk.</p>
        ...
        <form action={uploadCsv} className="rounded-[18px] border border-white/10 bg-[#1F2329] p-5">
          ...
          <button className="mt-3 rounded border border-white/15 px-3 py-2 text-sm font-medium hover:bg-white/5">Upload</button>
        </form>
      </div>
      ...
      <ul className="mt-3 divide-y divide-white/5 rounded-[18px] border border-white/10 bg-[#1F2329]">
        {batches.length === 0 && <li className="p-4 text-sm text-white/55">No batches yet — sync or upload above.</li>}
        {batches.map((b) => (
          <li key={b.id}>
            <Link href={`/batches/${b.id}`} className="flex items-center justify-between p-4 text-sm hover:bg-white/5">
              <span className="font-medium">{b.label}</span>
              <span className="text-white/40">{b.source} · {b.createdAt.toISOString().slice(0, 10)}</span>
            </Link>
          </li>
        ))}
      </ul>
