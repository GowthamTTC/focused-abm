...
export async function startEventScan(formData: FormData) {
  ...
  if (!metroBySlug(metro) || !isUsMetro(metro)) redirect(qs("sf-bay-area", 7, { err: "metro" }));
  if (pool === "extended" && !eventName) {
    redirect(qs(metro, days, { pool, err: "event" }));
  }
  if (pool === "extended") {
  ...
  redirect(qs(metro, days, { pool, scanning: "1" }));
}

export async function markFloor(id: string, status: "met" | "skipped", metro: string, days: number, pool = "first") {
  const user = await requireUser();
  await db.update(connection).set({
    floorStatus: status,
    floorAt: new Date(),
  }).where(and(eq(connection.orgId, user.orgId), eq(connection.id, id)));
  redirect(qs(metro, days, { pool, p: id }));
}

export async function clearFloor(id: string, metro: string, days: number, pool = "first") {
  const user = await requireUser();
  await db.update(connection).set({
    floorStatus: null,
    floorAt: null,
