    ...
    <Shell user={user} active="health">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-[16px] border border-[#263BAA]/12 bg-white p-6">
          ...
          <p className="mt-0.5 text-xs text-[#2B3355]/45">Of the {s.scanned.toLocaleString()} people with observed activity.</p>
          <div className="mt-4">
        ...
        <div className="rounded-[16px] border border-[#263BAA]/12 bg-white p-6">
          ...
          <p className="mt-0.5 text-xs text-[#2B3355]/45">Tier-1/2 prospects, never contacted, quiet 30+ days or unscanned.</p>
          <ul className="mt-3 divide-y divide-[#263BAA]/8">
            {attention.map((p) => (
              <li key={p.id} className="flex items-center gap-3 py-2.5 text-sm">
                <span className={`rounded px-1.5 py-0.5 text-[11px] font-semibold ${p.tier === 1 ? "bg-[#263BAA]/15 text-[#263BAA]" : "bg-[#263BAA]/8 text-[#2B3355]/60"}`}>T{p.tier}</span>
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium">{p.firstName} {p.lastName}</p>
                  <p className="truncate text-xs text-[#2B3355]/50">{p.companyRaw} · {p.lastPostAt ? `last post ${p.lastPostAt.toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : "activity unknown"}</p>
                </div>
                {p.batchId && <Link href={`/batches/${p.batchId}?view=pitchable&p=${p.id}`} className="text-xs text-[#263BAA] underline">open</Link>}
              </li>
            ))}
            {attention.length === 0 && <p className="py-3 text-sm text-[#2B3355]/45">Nothing slipping — every high-tier prospect is either fresh or already contacted.</p>}
          </ul>
        </div>
      </div>
    </Shell>
