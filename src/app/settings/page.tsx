    ...
    <Shell user={user} active="settings">
      ...
      <section className="mt-6 rounded-2xl glass border-0 p-5">
        ...
        <form action={saveEnrichLimit} className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <button className="rounded-lg bg-[#263BAA] px-4 py-2 text-sm font-semibold text-[#14204A] hover:bg-[#1D2E86]">Save</button>
          {saved && <span className="text-sm text-[#46506E]/55">Guardrail saved.</span>}
        </form>
        ...
        <p className="mt-3 text-xs text-[#46506E]/35">
      ...
      <section className="mt-6 rounded-2xl glass border-0 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <form action={startConnect}>
            <button className="rounded-lg bg-[#263BAA] px-4 py-2 text-sm font-semibold text-[#14204A] shadow-[0_0_18px_rgba(38,59,170,.2)] hover:bg-[#1D2E86]">
              Connect LinkedIn
            </button>
          </form>
        </div>
          ...
          {accounts.map((a) => (
            <li key={a.id} className="flex items-center justify-between gap-4 p-4 text-sm">
              <div className="min-w-0">
                <p className="tnum truncate text-[#14204A]">{a.displayName ?? a.unipileAccountId}</p>
                <p className="tnum mt-0.5 text-xs text-[#46506E]/35">connected {a.createdAt.toISOString().slice(0, 10)}</p>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                <span className={`rounded px-2 py-0.5 text-xs ${
                  a.status === "operational" ? "bg-[#263BAA]/15 text-[#263BAA]"
                  : a.status === "needs_reauth" ? "bg-[#B54708]/15 text-[#B54708]"
                  : "bg-[#263BAA]/10 text-[#46506E]/50"}`}>
                  {a.status === "needs_reauth" ? "needs re-auth" : a.status}
                </span>
                {a.status === "needs_reauth" && (
                  <form action={startConnect}>
                    <button className="rounded-lg border border-[#D0D5E4] px-3 py-1 text-xs hover:bg-[#263BAA]/5">Reconnect</button>
                  </form>
                )}
                <form action={refreshStatus.bind(null, a.unipileAccountId)}>
                  <button className="text-xs text-[#46506E]/55 hover:text-[#14204A]">Refresh</button>
                </form>
        ...
        {!unipileConfigured && (
          <p className="mt-3 text-sm text-[#46506E]/45">Running in mock mode — add Unipile keys to go live.</p>
        )}
      </section>
    </Shell>
  );
