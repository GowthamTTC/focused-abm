      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        {services.map((s) => (
          <Link key={s.id} href={`/services/${s.slug}`}
            className="rounded-2xl glass border-0 p-5 transition hover:border-[#263BAA]/30">
        ...
        <Link href="/services/new"
          className="flex min-h-[140px] items-center justify-center rounded-2xl border border-dashed border-[#D0D5E4] text-sm text-[#46506E]/45 hover:border-[#263BAA]/40 hover:text-[#263BAA]">
          + Add another offer
        </Link>
      </div>
    </Shell>
