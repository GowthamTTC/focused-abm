...
function StatusChip({ s }: { s: string }) {
  const cls = s === "done" ? "bg-[#263BAA]/15 text-[#263BAA]"
    : s === "running" ? "bg-[#B07818]/15 text-[#B07818]"
    : s === "failed" ? "bg-red-500/15 text-red-600"
    : "bg-[#263BAA]/10 text-[#2B3355]/60";
    ...
    <Shell user={user} active="connections">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="tnum mt-1 text-[#2B3355]/45">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {counts.unclassified > 0 && (
            <form action={runClassify.bind(null, id)}>
              <button className="rounded-lg bg-[#263BAA] px-3 py-2 text-sm font-semibold text-[#1B2559] shadow-[0_0_18px_rgba(38,59,170,.25)] hover:bg-[#1D2E86]">
          ...
          {counts.unclassified === 0 && classifiedRows > 0 && (
            <form action={runClassify.bind(null, id)}>
              <button title="Recompute scores, tiers and ranks — zero model calls."
                className="rounded-lg border border-[#263BAA]/20 px-3 py-2 text-sm text-[#2B3355]/70 hover:bg-[#263BAA]/5">Re-rank</button>
            </form>
          )}
          {classifiedRows > 0 && (
            <form action={reclassifyAllAction.bind(null, id)}>
              <button title="Re-run Stage A on every row — use after ICP/prompt edits."
                className="rounded-lg border border-[#263BAA]/20 px-3 py-2 text-sm text-[#2B3355]/70 hover:bg-[#263BAA]/5">Reclassify all</button>
            </form>
          )}
          {counts.pitchable > 0 && (
            <form action={scanActivity.bind(null, id)} className="flex items-center gap-2">
              ...
              <input name="n" type="number" defaultValue={50} min={1} max={200}
                className="tnum w-16 rounded-lg border border-[#263BAA]/12 bg-[#FBF3DE] px-2 py-1.5 text-xs" />
          ...
          {counts.pitchable > 0 && (
            <form action={selectTopN.bind(null, id)} className="flex items-center gap-2">
              <select name="n" defaultValue={defaultN}
                className="rounded-lg border border-[#263BAA]/20 bg-[#FBF3DE] px-2 py-2 text-sm">
              ...
              <button title="Queue the next N un-enriched people by rank — already-enriched people are never re-taken."
                className="rounded-lg bg-[#263BAA] px-3 py-2 text-sm font-semibold text-[#1B2559] hover:bg-[#1D2E86]">
                Select next N
              </button>
              <Link href="/settings" className="text-xs text-[#2B3355]/40 underline decoration-white/20 hover:text-[#2B3355]/80"
            ...
            <form action={runDeepEnrich.bind(null, id)}>
              <button disabled={selQueued === 0}
                ...
                className={selQueued > 0
                  ? "rounded-lg bg-[#263BAA] px-3 py-2 text-sm font-semibold text-[#1B2559] hover:bg-[#1D2E86]"
                  : "cursor-not-allowed rounded-lg border border-[#263BAA]/12 bg-[#263BAA]/5 px-3 py-2 text-sm text-[#2B3355]/30"}>
      ...
      <div className="mt-5">
        ...
        <div className="mt-1.5 flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 text-xs">
          <span className="text-[#2B3355]/35">One tick per connection, rank order — lime Top-N ignites as enrichment completes.</span>
      ...
      {/* Tabs */}
      <nav className="mt-6 flex gap-4 border-b border-[#263BAA]/12 text-sm">
      ...
      {view === "enriched" ? (
        ...
        rows.length === 0 ? (
          <div className="mt-6 rounded-[18px] border border-dashed border-[#263BAA]/12 p-10 text-center text-sm text-[#2B3355]/55">
          ...
          <div className="mt-6 flex flex-col gap-5 lg:flex-row">
            ...
            {person && (
              <div className="min-w-0 flex-1 rounded-[18px] border border-[#263BAA]/12 bg-white p-6">
                ...
                {/* Stage A */}
                <h3 className="mt-6 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#2B3355]/45">
                  <span className="h-2.5 w-2.5 rounded-sm bg-white/25" /> Stage A — matched from metadata
                </h3>
                <div className="mt-2 grid grid-cols-1 gap-x-8 gap-y-3 rounded-xl border border-[#263BAA]/12 bg-[#FBF3DE] p-4 text-sm md:grid-cols-2">
                  <div><p className="text-xs text-[#2B3355]/45">Company</p><p className="mt-0.5">{person.companyRaw ?? "—"}</p></div>
                  <div><p className="text-xs text-[#2B3355]/45">Position</p><p className="mt-0.5">{person.positionRaw ?? person.headlineRaw ?? "—"}</p></div>
                  <div><p className="text-xs text-[#2B3355]/45">Provisional service</p><p className="tnum mt-0.5">{person.serviceSlug ?? "—"}</p></div>
                  <div><p className="text-xs text-[#2B3355]/45">Why</p><p className="mt-0.5 text-[#2B3355]/80">{person.matchWhy ?? "—"}</p></div>
                </div>

                {/* Stage B */}
                <h3 className="mt-6 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#B07818]/80">
                  <span className="h-2.5 w-2.5 rounded-sm bg-[#B07818]" /> Stage B — read from profile
                </h3>
                {person.enrichStatus === "failed" ? (
                  ...
                  <p className="mt-2 rounded-xl border border-[#263BAA]/12 bg-[#FBF3DE] p-4 text-sm text-[#2B3355]/55">
                    {person.enrichStatus === "running" ? "Reading profile now…" : "Queued — press Deep enrich to run."}
                  </p>
                ) : (
                  <div className="mt-2 space-y-5 rounded-xl border border-[#B07818]/15 bg-[#B07818]/[.04] p-4 text-sm">
                    <div>
                      <p className="text-xs text-[#B07818]/70">About — summary</p>
                      <p className="mt-1 text-[#2B3355]/85">{person.aboutSummary ?? "—"}</p>
                    </div>
                    <div>
                      <p className="text-xs text-[#B07818]/70">
                        Posts{" "}
                      ...
                      {person.postsSummary
                        ? <p className="mt-1 text-[#2B3355]/85">{person.postsSummary}</p>
                        : <p className="mt-1 text-[#B07818]">No original posts found — summary generated from profile only.</p>}
                    </div>
                    <div>
                      <p className="text-xs text-[#B07818]/70">
                        Pain points{" "}
                        {person.painInferred && (
                          <span className="rounded border border-[#B07818]/50 px-1.5 py-px text-[10px] text-[#B07818]">inferred</span>
                        )}
                      </p>
                      <p className="mt-1 text-[#2B3355]/85">{person.painPoints ?? "—"}</p>
                    </div>
                    <div>
                      <p className="text-xs text-[#B07818]/70">Service to pitch</p>
                      <p className="mt-1">
                        ...
                        {person.correctionReason && <span className="text-[#2B3355]/60"> — {person.correctionReason}</span>}
                      </p>
                      {person.flag && (
                        <p className="mt-1.5">
                          <span className="rounded border border-red-400/50 px-1.5 py-px text-[10px] text-red-600">⚑ flag</span>
                          <span className="ml-2 text-[#2B3355]/70">{person.flag}</span>
                        </p>
                      )}
                    </div>
        ...
        <div className="mt-6 rounded-[18px] border border-dashed border-[#263BAA]/12 p-12 text-center">
          {view === "pitchable" && counts.unclassified > 0 ? (
            ...
            <p className="text-sm text-[#2B3355]/55">Nothing here yet.</p>
          )}
        </div>
      ) : (
        /* ── Bucket tables ── */
        <div className="mt-4 overflow-x-auto rounded-[18px] border border-[#263BAA]/12 bg-white">
          <table className="w-full text-left text-sm">
            <thead className="bg-[#263BAA]/5 text-xs uppercase tracking-wide text-[#2B3355]/45">
              <tr>
                {view === "pitchable" ? (
            ...
            <tbody className="divide-y divide-[#263BAA]/8 align-top">
              {rows.map((c) => {
                ...
                return (
                  <tr key={c.id} className={c.selectedForEnrich && view === "pitchable"
                    ...
                    {view === "pitchable" ? (<>
                      ...
                      <td className="px-3 py-2.5">
                        {c.tier && (
                          <span className={`rounded px-1.5 py-0.5 text-[11px] font-semibold ${
                            c.tier === 1 ? "bg-[#263BAA]/20 text-[#263BAA]" : "bg-[#263BAA]/10 text-[#2B3355]/60"}`}>
                            T{c.tier}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        {c.serviceSlug && <span className="rounded bg-[#263BAA]/10 px-1.5 py-0.5 text-[11px] text-[#2B3355]/75">{c.serviceSlug}</span>}
                      </td>
                      <td className="relative max-w-sm px-3 py-2.5 text-xs text-[#2B3355]/55">
                        <div className="group">
                          ...
                          <div className="pointer-events-none absolute left-0 top-full z-20 mt-1 hidden w-[26rem] max-w-[80vw] rounded-xl border border-[#263BAA]/12 bg-white/95 p-3.5 shadow-2xl backdrop-blur group-hover:block">
                            ...
                            <span className="mt-2 inline-block rounded border border-[#263BAA]/20 px-1.5 py-0.5 text-[10px] text-[#2B3355]/60">
                              {c.matchMethod === "rule" ? "rule pass" : c.matchMethod === "manual" ? "manual" : "model pass"}
                            </span>
                          </div>
                        </div>
                      </td>
                      <td className="tnum px-3 py-2.5 text-right text-[#2B3355]/70">{c.score ?? "—"}</td>
                    </>) : (<>
                      <td className="px-3 py-2.5 font-medium">
                        {c.linkedinUrl
                          ? <a href={c.linkedinUrl} target="_blank" className="underline decoration-white/20 hover:decoration-[#263BAA]">{c.firstName} {c.lastName}</a>
                      ...
                      <td className="px-3 py-2.5">{c.companyRaw}</td>
                      <td className="px-3 py-2.5">{c.positionRaw ?? c.headlineRaw}</td>
                      <td className="max-w-md px-3 py-2.5 text-xs text-[#2B3355]/55">{c.matchWhy}</td>
                      <td className="px-3 py-2.5 text-right">
                        {(view === "off_icp" || view === "peer_competitor") && (
                          <form action={moveToPitchable.bind(null, id, c.id)}>
                            <button className="whitespace-nowrap text-xs text-[#2B3355]/35 hover:text-[#263BAA]">
                              Move to pitchable
                            </button>
                          </form>
                        )}
