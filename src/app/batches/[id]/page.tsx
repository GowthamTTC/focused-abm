    ...
    <Shell user={user} active="connections">
        ...
        <div className="flex items-center gap-2">
          ...
          {counts.unclassified === 0 && classifiedRows > 0 && (
            <form action={runClassify.bind(null, id)}>
              <button title="Recompute scores, tiers and ranks without any model calls — use after scoring or ICP changes."
                className="rounded border border-white/15 px-3 py-2 text-sm text-white/70 hover:bg-white/5">
                Re-rank
              </button>
            </form>
          )}
          ...
          {counts.pitchable > 0 && (
            <form action={selectTopN.bind(null, id)} className="flex items-center gap-2">
              ...
              <button className="rounded border border-white/15 px-3 py-2 text-sm font-medium hover:bg-white/5">Select top N</button>
              <Link href="/settings" className="text-xs text-white/40 hover:text-white/80"
          ...
          <form action={`/api/export/${id}`} method="get" className="flex items-center gap-2">
            <label className="flex items-center gap-1.5 text-xs text-white/55" title="Adds an internal diagnostics tab: score breakdown, confidence, rule/model provenance, enrichment status, flags. Leave off for client-facing exports.">
      ...
      <nav className="mt-6 flex gap-3 border-b border-white/10 text-sm">
        {["pitchable", "enriched", "off_icp", "peer_competitor", "excluded"].map((v) => (
          <Link key={v} href={`/batches/${id}?view=${v}`}
            className={`-mb-px border-b-2 px-1 pb-2 ${view === v ? "border-[#B6FF2E] font-medium" : "border-transparent text-white/55 hover:text-[#E8EAF0]"}`}>
      ...
      <div className="mt-4 overflow-x-auto rounded-[18px] border border-white/10 bg-[#1F2329]">
        <table className="w-full text-left text-sm">
          <thead className="bg-white/10 text-xs uppercase tracking-wide text-white/55">
            <tr>
          ...
          <tbody className="divide-y divide-white/5 align-top">
            {rows.map((c) => (
              <tr key={c.id} className={c.selectedForEnrich && view === "pitchable" ? "bg-[#B6FF2E]/5" : ""}>
                <td className="px-3 py-2 text-white/40">{c.rank ?? "—"}</td>
                ...
                {view === "enriched" ? (<>
                  <td className="px-3 py-2">
                    <span className={`rounded-full px-2 py-0.5 text-xs ${
                      ...
                      : "bg-white/10 text-white/70"}`}>{c.enrichStatus}</span>
                    {c.flag && <p className="mt-1 text-xs text-red-400">⚑ {c.flag}</p>}
                  </td>
                  <td className="px-3 py-2 text-xs">{c.serviceConfirmed}</td>
                  <td className="max-w-md px-3 py-2 text-xs text-white/70">{c.outreachMessage}</td>
                </>) : (<>
                  <td className="px-3 py-2">{c.tier ? `T${c.tier}` : ""}</td>
                  <td className="px-3 py-2 text-xs">{c.serviceSlug}</td>
                  <td className="max-w-md px-3 py-2 text-xs text-white/55" title={JSON.stringify(c.scoreBreakdownJson)}>{c.matchWhy}</td>
                </>)}
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={7} className="px-3 py-8 text-center text-sm text-white/55">
