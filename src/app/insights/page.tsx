...
export default async function InsightsPage() {
  ...
  return (
    <Shell user={user} active="insights">
      ...
      <p className="mt-1 text-sm text-[#2B3355]/55">The structure and quality of your network — every number traces to observed data.</p>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="rounded-[16px] border border-[#263BAA]/12 bg-white p-6">
          ...
          <p className="mt-0.5 text-xs text-[#2B3355]/45">One observed point per day — history begins the day this page first loaded.</p>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-[16px] border border-[#263BAA]/12 bg-white p-6">
          ...
          <p className="mt-0.5 text-xs text-[#2B3355]/45">Which of your offers your network maps to.</p>
          <div className="mt-4"><HBars items={services.map((x) => ({ label: x.slug, n: x.n }))} /></div>
        </div>
        <div className="rounded-[16px] border border-[#263BAA]/12 bg-white p-6">
          <h2 className="font-medium">Top locations</h2>
          <p className="mt-0.5 text-xs text-[#2B3355]/45">From synced profile locations; CSV-imported rows have none.</p>
          <div className="mt-4">
            {countries.length > 0 ? <HBars items={countries.map((x) => ({ label: x.country, n: x.n }))} />
              : <p className="text-sm text-[#2B3355]/45">No location data yet — locations arrive with LinkedIn sync.</p>}
          </div>
        </div>
      </div>
    </Shell>
