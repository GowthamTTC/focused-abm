  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)}
        className="rounded-[8px] border border-emerald-400/50 bg-emerald-500/10 px-3 py-2 text-sm font-medium text-emerald-300 hover:bg-emerald-500/20">
        Export workbook
      </button>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 -sm"
          onClick={() => setOpen(false)}>
          <div className="w-full max-w-md bg-white border border-[#DDE2EE] rounded-[14px] shadow-[0_1px_2px_rgba(16,24,40,.04)] p-5"
            onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold">Export workbook</h2>
            <div className="mt-4 bg-white border border-[#DDE2EE] rounded-[10px] shadow-[0_1px_2px_rgba(16,24,40,.04)] p-4 text-sm">
              {([["Instructions", "—"], [`Top ${topN || "N"}`, topN || "—"], ["Target Pool", targetPool.toLocaleString()], ["Review", review.toLocaleString()], ["Peers", peers.toLocaleString()]] as const)
                .map(([k, v]) => (
                  <div key={k} className="flex justify-between py-1">
                    <span className="text-[#475467]">{k}</span>
                    <span className="tnum text-[#101828]">{v}</span>
                  </div>
                ))}
              {ops && (
                <div className="flex justify-between py-1">
                  <span className="text-[#475467]">Ops</span>
                  <span className="tnum text-[#98A2B3]">full diagnostics</span>
                </div>
              )}
            </div>
            <p className="mt-3 flex items-center gap-4 text-xs text-[#98A2B3]">
            ...
            <a href={`/api/export/${batchId}${ops ? "?ops=1" : ""}`} onClick={() => setOpen(false)}
              className="mt-5 block rounded-[10px] bg-emerald-400 py-3 text-center text-sm font-semibold text-[#101828] hover:bg-emerald-300">
              Download .xlsx
            </a>
          </div>
        </div>
